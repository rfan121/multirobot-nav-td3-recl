...
class GazeboEnv:
    ...
    def change_goal(self):
        """Randomly assigns new goal positions for all robots, ensuring they are not on obstacles and not too close to each other."""
        ...
        while not goal_ok:
            ...
            # Check all goals are on valid positions
            valid_positions = all(check_pos(self.goal_x[i], self.goal_y[i]) for i in range(self.num_robots))

            # Check pairwise distances between all robot goals
            sufficient_spacing = True
            for i in range(self.num_robots):
                for j in range(i + 1, self.num_robots):
                    dx = self.goal_x[i] - self.goal_x[j]
                    dy = self.goal_y[i] - self.goal_y[j]
                    distance = (dx**2 + dy**2)**0.5
                    if distance < 1.0:
                        sufficient_spacing = False
                        break
                if not sufficient_spacing:
                    break

            goal_ok = valid_positions and sufficient_spacing
